  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#0b1d33] py-12 sm:py-14 lg:py-16 border-t border-white/5">
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        <p className="text-xs sm:text-sm font-bold uppercase tracking-[0.2em] text-white">More from Trijotech</p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
          {others.map((solution) => (
            <article
              key={solution.slug}
              className="group flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] shadow-md transition-all duration-300 hover:-translate-y-1 hover:border-white/60 hover:bg-white/[0.05] hover:shadow-[0_16px_40px_rgba(255, 255, 255,0.18)]"
            >
              ...
              <div className="flex flex-1 flex-col p-6">
                <h3 className="text-xl font-bold text-white group-hover:text-white transition-colors">{solution.title}</h3>
                <p className="mt-2 flex-1 text-sm leading-6 text-slate-400">{solution.shortDescription}</p>
                <Link href={solution.href} className="mt-auto inline-flex items-center gap-2 pt-5 font-semibold text-white transition-all duration-200 group-hover:gap-3 group-hover:text-white">
