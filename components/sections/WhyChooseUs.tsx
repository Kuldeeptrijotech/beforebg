  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#18263e] py-12 sm:py-14 lg:py-16 text-white border-t border-white/10">
      ...
      <Container className="relative">
        ...
        {/* stats band */}
        <Reveal delay={0.08}>
          <div className="mt-6 sm:mt-8 grid gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur-md sm:grid-cols-2 lg:grid-cols-4 sm:p-6">
            {statValues.map((stat) => (
              ...
              <StaggerRevealItem key={item.title} className="h-full">
                <article className="tri-glass-card group flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h3 className="text-base sm:text-lg font-bold leading-snug text-white group-hover:text-[#7edcc2] transition-colors">{item.title}</h3>
