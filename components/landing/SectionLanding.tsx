...
export default function SectionLanding({ eyebrow, title, description, heroImage, cardsTitle, cardsIntro, cards }: Props) {
  return (
        ...
        <div className="mx-auto flex min-h-[580px] max-w-7xl items-center px-5 py-16 sm:px-8 lg:px-12">
          <div className="max-w-3xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-cyan-50 backdrop-blur-md">
              <Sparkles className="h-4 w-4 text-cyan-200" /> {eyebrow}
            </div>
            <h1 className="max-w-3xl text-4xl font-bold leading-[1.08] tracking-tight text-white sm:text-5xl lg:text-7xl">{title}</h1>
            <p className="mt-6 max-w-2xl text-xl font-medium leading-8 text-cyan-50 sm:text-2xl">{description}</p>
            <div className="mt-9 flex flex-wrap gap-4">
              <a href="#explore" className="inline-flex items-center gap-2 rounded-full bg-cyan-400 px-6 py-3.5 font-semibold text-cyan-950 shadow-lg shadow-cyan-950/20 transition hover:-translate-y-0.5 hover:bg-cyan-300">Explore {eyebrow} <ArrowRight className="h-4 w-4" /></a>
              <Link href="/contact" className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/10 px-6 py-3.5 font-semibold text-white backdrop-blur-md transition hover:bg-white/20">Contact Us</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="explore" className="relative bg-gradient-to-b from-[#d5eafa] to-[#e8f2fb] py-20 sm:py-24">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-3xl font-bold tracking-tight text-slate-900 sm:text-5xl">{cardsTitle}</h2>
            <p className="mt-5 text-lg leading-8 text-slate-600">{cardsIntro}</p>
          </div>
              ...
              <article key={card.href} className="group flex min-w-0 flex-col overflow-hidden rounded-3xl border border-blue-200 bg-[#f5faff] shadow-sm transition duration-300 hover:-translate-y-1 hover:border-cyan-300 hover:shadow-xl hover:shadow-cyan-950/10">
                ...
                <div className="flex flex-1 flex-col p-6 sm:p-7">
                  ...
                  {card.capabilities?.length ? (
                    <ul className="mt-5 space-y-2 border-t border-cyan-100 pt-5">
                      {card.capabilities.slice(0, 4).map((item) => <li key={item} className="flex items-start gap-2 text-sm font-medium text-slate-700"><Check className="mt-0.5 h-4 w-4 shrink-0 text-cyan-700" />{item}</li>)}
                    </ul>
                  ) : null}
                  <Link href={card.href} className="mt-7 inline-flex items-center gap-2 font-semibold text-cyan-700 transition group-hover:gap-3 group-hover:text-cyan-600">{card.cta ?? "Explore"} <ArrowRight className="h-4 w-4" /></Link>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="bg-[#ccdfef] px-5 py-20 sm:px-8 sm:py-24 lg:px-12">
        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-gradient-to-br from-cyan-700 to-cyan-950 px-6 py-14 text-center shadow-2xl shadow-cyan-950/15 sm:px-12 sm:py-20">
          ...
          <div className="relative mx-auto max-w-3xl">
            ...
            <h2 className="mt-4 text-3xl font-bold tracking-tight text-white sm:text-5xl">Let's turn your next priority into measurable progress.</h2>
            <Link href="/contact" className="mt-9 inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 font-semibold text-cyan-950 shadow-lg transition hover:-translate-y-0.5 hover:bg-cyan-50">Talk to our team <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
    </main>
