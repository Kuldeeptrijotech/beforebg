  ...
  return (
    <SceneCanvas bleed className="h-full w-full">
      <svg aria-hidden className="absolute inset-0 h-full w-full" viewBox="0 0 640 480" preserveAspectRatio="none" fill="none">
        <FlowLink d={ZIG} color="rgba(255, 255, 255,0.28)" width={2} dash />
        <FlowLink d={COLUMN} color="rgba(255, 255, 255,0.35)" width={1.5} dash />
        ...
        {TICKS.map((t, i) => (
          <line key={i} x1={t.x} y1={t.y - 4} x2={t.x} y2={t.y + 4} stroke="rgba(255, 255, 255,0.35)" strokeWidth={1.4} />
        ))}
        {NODES.map((n) => (
          <PulseDot key={n.label} cx={n.x} cy={n.y} color={n.tone === "amber" ? TRI.amber : TRI.green} r={3} />
        ))}
        <polygon points="360,239 371,250 360,261 349,250" fill="rgba(255, 255, 255,0.18)" stroke={TRI.green} strokeWidth={1.4} />
        <path d="M355 250 l4 4 l8 -8" stroke={TRI.green} strokeWidth={1.6} fill="none" strokeLinecap="round" />
        <rect x={192} y={202} width={36} height={36} rx={4} fill="rgba(26,15,6,0.9)" stroke="rgba(255, 255, 255,0.55)" strokeWidth={1.4} />
        <rect x={198} y={208} width={24} height={24} rx={2} fill="rgba(255, 255, 255,0.14)" className="tri-pulse" />
        <rect x={204} y={188} width={12} height={14} fill="rgba(255, 255, 255,0.35)" />
        <rect x={204} y={184} width={12} height={4} fill="rgba(255, 255, 255,0.2)" />
        {BARS.map((b, i) => (
          <rect key={i} x={b.x} y={b.y} width={10} height={b.h} rx={2} fill={i === 1 ? TRI.amber : "rgba(255, 255, 255,0.7)"} className="tri-pulse" style={{ animationDelay: `${i * 0.6}s` }} />
        ))}
        <GlowHalo cx={210} cy={220} r={44} color={TRI.amber} opacity={0.2} />
        <GlowHalo cx={560} cy={90} r={30} color={TRI.amber} opacity={0.16} />
        <SceneText x={210} y={276} size={9} fill="rgba(255, 255, 255,0.7)">HOT ZONE</SceneText>
        <SceneText x={560} y={70} size={9} fill="rgba(255, 255, 255,0.65)">MONTH-END CLOSE</SceneText>
      </svg>
      ...
      <div className="absolute right-[7%] top-[70%] z-10 flex items-center gap-3 rounded-xl border border-white/10 bg-black/30 px-4 py-3 shadow-lg shadow-black/40 backdrop-blur-md">
        ...
        <div className="leading-tight">
          ...
          <p className="text-sm font-bold text-white">96.4% <span className="font-medium text-white/70">on spec</span></p>
        </div>
      </div>
    </SceneCanvas>
  );
