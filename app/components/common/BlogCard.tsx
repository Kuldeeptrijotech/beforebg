...
export default function BlogCard({ blog, variant = "listing" }: BlogCardProps) {
  if (variant === "carousel") {
    return (
      <article className="flex min-h-[450px] w-[clamp(280px,29vw,370px)] shrink-0 basis-[clamp(280px,29vw,370px)] flex-col overflow-hidden rounded-3xl border border-white/12 bg-[#1a2336]/90 shadow-2xl backdrop-blur-md sm:min-h-[470px] max-[640px]:w-[82vw] max-[640px]:basis-[82vw] transition-all hover:border-[#29ab87]/50">
        ...
        <div className="flex min-h-0 flex-1 flex-col bg-transparent p-5">
          <span className="text-[13px] font-bold text-[#f5a623]">{blog.date}</span>
          <h3 className="my-[9px] line-clamp-2 min-h-[53px] max-h-[53px] overflow-hidden text-[18px] font-bold leading-[1.45] text-white">
            <Link href={blog.link} className="text-white hover:text-[#7edcc2] transition-colors">{blog.title}</Link>
          </h3>
          <p className="mb-[14px] line-clamp-3 max-h-[67px] overflow-hidden text-[14px] leading-[1.6] text-slate-300">{blog.description}</p>
          <Link href={blog.link} className="mt-auto text-[14px] font-bold text-[#7edcc2] hover:text-[#f5a623] transition-colors">Read article →</Link>
        </div>
      </article>
    );
  }
  ...
  return (
    <article className="flex min-h-[460px] w-full flex-col overflow-hidden rounded-3xl border border-white/12 bg-[#1a2336]/90 p-0 shadow-2xl backdrop-blur-md transition-all hover:border-[#29ab87]/50">
      ...
      <div className="flex min-h-[238px] flex-1 flex-col p-5">
        <div className="text-[#f5a623] text-xs font-bold uppercase"><span>{blog.date}</span></div>
        ...
        <Link href={blog.link} className="mt-auto inline-flex w-fit items-center gap-1 text-sm font-bold text-[#7edcc2] hover:text-[#f5a623] transition-colors">Read More →</Link>
      </div>
    </article>
  );
}
