...
export default function BlogCard({blog,variant="listing"}:BlogCardProps){
  ...
  return <article className="service-item blog-card"><Link href={blog.link} className="blog-card-image" aria-label={`Read blog: ${blog.title}`}><img loading="lazy" src={blog.image} alt={blog.title} className="img-fluid"/></Link><div className="down-content blog-card-content"><div className="blog-meta"><span><i className="fa fa-calendar" aria-hidden="true"/> {blog.date}</span></div><h4>{blog.title}</h4><p>{blog.description}</p><Link href={blog.link} className="filled-button read-more">Read More</Link></div></article>
}