...
const getLegacyPage = (slug: string) => legacyBlogPages[slug as Slug] ?? Object.entries(legacyBlogPages).find(([key]) => asciiSlug(key) === slug)?.[1];

export const dynamic = "force-dynamic";

export async function generateMetadata({ params }: PageProps<"/blogs/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const managed = (await readBlogPosts()).find((post) => post.slug === slug && post.status === "published");
  if (managed) return { title: managed.seoTitle || managed.title, description: managed.seoDescription || managed.shortDescription };
  const legacy = getLegacyPage(slug);
  return legacy ? { title: legacy.title, description: legacy.description || undefined } : {};
}

export default async function Page({ params, searchParams }: PageProps<"/blogs/[slug]">) {
  const [{ slug }, query, cookieStore, persistent, posts] = await Promise.all([params, searchParams, cookies(), hasPersistentBlogStore(), readBlogPosts()]);
  const managed = posts.find((post) => post.slug === slug);
  const adminPreview = query.adminPreview === "1" && verifySessionToken(cookieStore.get(SESSION_COOKIE)?.value);
  const hasManagedContent = Boolean(managed?.content?.trim() || managed?.contentBlocks?.length);
