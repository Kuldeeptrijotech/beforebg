...
export default function ImageUploadField({ id, label, value, alt, onUploaded, onError }: Props) {
  ...
  return (
    <section className="admin-image-manager" aria-labelledby={`${id}-label`}>
      ...
      {value ? <>
        {/* Admin previews may use arbitrary validated remote URLs, so a native image is intentional here. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <div className="admin-image-preview"><img src={value} alt={alt || `${label} preview`} /></div>
      </> : <div className="admin-image-preview admin-image-preview-empty">No image selected</div>}
      <div
        className={`admin-image-dropzone ${dragging ? "is-dragging" : ""}`}
      ...
      >
        ...
        <button type="button" className="admin-secondary-button" onClick={() => input.current?.click()} disabled={uploading} aria-label={`${value ? "Replace" : "Upload"} ${label}`}>
          {uploading ? <><span className="admin-upload-spinner" aria-hidden="true" /> Uploading…</> : value ? "Replace Image" : "Upload New Image"}
        </button>
      </div>
    </section>
  );
