...
export async function POST(request: Request) {
  try {
    ...
    const message = text(form, "message", 5000), captcha = text(form, "captchaToken", 2048), consent = text(form, "consent", 10);
    if (name.length < 2 || !validEmail(email) || !validPhone(phone) || subject.length < 3 || message.length < 10 || consent !== "yes") return NextResponse.json({ error: "Please complete all required fields with valid information." }, { status: 400 });
    if (!(await verifyCaptcha(captcha, ip, "contact_form"))) return NextResponse.json({ error: "Human verification failed. Please try again." }, { status: 400 });
